...
%DIF ADD AurigaGCS.tex            Sat Apr 11 11:18:43 2020
%
...
model produces enough stellar mass with the right properties\DIFdelbegin \DIFdel{to allow for formation efficiencies lower than unity and
...
%DIF >  Referee: ``\citet{2012MNRAS.426.1475U} shows the great variety in GC metallicity distributions
%DIF >  present in the sample of early-type galaxies of the SLUGGS survey contradicts
%DIF >  the classical idea that all the metallicity distributions of GCs should be bimodal.''
\DIFaddbegin 


\DIFaddend \section{The Auriga simulations}
\label{sec:auriga}
...
}\DIFaddend were selected from the \mbox{\textsc{eagle}} DM only box, see 
...
from the observed positions }\DIFaddend RA and DEC, further discussed in Sec.~\ref{sec:Rgc}.
...
subpopulation of halo GCs. \DIFaddbegin \DIFadd{Our naive expectation would be that the spiral galaxies
in the Local Group host similar GC systems. }\DIFaddend However, the \DIFdelbegin \DIFdel{two radial distributions }\DIFdelend \DIFaddbegin \DIFadd{radial distributions of
the GCS of the MW and M31 }\DIFaddend are not statistically consistent with being drawn from 
...
when no additional selection criterion is applied). \DIFaddbegin \DIFadd{The mean number of star 
...
age cut neglects the more metal-rich stellar population which shifts the mean
...
metallicity in this simulations }\DIFaddend $\mu = -1.51 \, ({-0.74})$ for the {\it in~situ} 
...
\DIFdelbegin \DIFdel{(magenta ) }\DIFdelend \DIFaddbegin \DIFadd{and magenta }\DIFaddend cross, which shows the mean value of all MW \DIFdelbegin \DIFdel{(}\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend M31 \DIFdelbegin \DIFdel{) GCs}\DIFdelend \DIFaddbegin \DIFadd{GCs, respectively}\DIFaddend . 
...
accreted populations. \DIFdelbegin \DIFdel{The Auriga simulations therefore do not support the view 
...
radially extended halo GC population. \DIFdelbegin \DIFdel{In fact, our classification might not hold
for Au4-1}\DIFdelend \DIFaddbegin \DIFadd{Our classification of accreted GC candidates
is thus not consistent with the aforementioned definition because our selection
function mostly includes star particles that have formed before roughly $z=2$}\DIFaddend .
...
whether the star formation model implemented for the Auriga simulations can give
...
has made use of NASA's Astrophysics Data System Bibliographic Services}\DIFaddend .
