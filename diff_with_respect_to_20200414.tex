...
%DIF DEL AurigaGCS_20200414.tex   Wed May 13 15:34:09 2020
%DIF ADD AurigaGCS.tex            Wed May 13 16:23:00 2020
%
...
is more metal-poor, more radially extended, and showing little to no rotation \DIFdelbegin \DIFdel{wheras
the later }\DIFdelend \DIFaddbegin \DIFadd{whereas
the latter }\DIFaddend is more metal-rich, radially concentrated, and rotating with the galaxy.
...
Yet another hypothesis is that star cluster formation \DIFdelbegin \DIFdel{is }\DIFdelend \DIFaddbegin \DIFadd{traces periods of high-intensity
star formation, which can be }\DIFaddend triggered by major gas-rich (spiral) galaxy mergers 
\citep{1987nngp.proc...18S, 1992ApJ...384...50A}\DIFdelbegin \DIFdel{,
which }\DIFdelend \DIFaddbegin \DIFadd{. Such a scenario }\DIFaddend is naturally 
expected within the framework of hierarchical assembly \DIFdelbegin \DIFdel{. One
testable prediction of this scenario is the }\DIFdelend \DIFaddbegin \DIFadd{and predicts }\DIFaddend formation 
...
populated by GCs, and the gravitational force softening can be as low \DIFaddbegin \DIFadd{as }\DIFaddend several 
...
2019MNRAS.486..331C,2019arXiv190505199C}, also done in the work by 
...
\DIFdelbegin \DIFdel{However, we do }\DIFdelend \DIFaddbegin \DIFadd{On the other hand, the MW could be an outlier as it may have formed and assembled 
most of its mass earlier than galaxies with a similar mass \mbox{%DIFAUXCMD
\citep[e.g.][]{2014ApJ...781L..31S,
2015A&A...578A..87S,2018MNRAS.477.5072M}}\hspace{0pt}%DIFAUXCMD
.
In addition, we }\DIFaddend caution that both data sets are incomplete and the age 
...
We show the two-dimensional mass-weighted metallicity-radius distribution of the
...
The observations indicate very few GCs with high metallicities at large radii (the three bins
in the upper right corner, both for MW and for M31), and relatively few GCs at large
radii in general ($r_{\text{gal}} > 30$~kpc; right column: $11$ GCs or 7.3\% in
the MW and $17$ or 4.6\% in M31). Moreover, M31 hosts more metal-rich ([Fe/H]$ > -1$) 
GCs in each radial bin in comparison to the MW after accounting for the fact that M31
hosts a larger GCS. Finally, given that the marginalized (i.e. the metallicity 
and radial) distributions are not statistically consistent between the two galaxies, 
we find that the two-dimensional distributions are also not consistent. More generally,
the GCS of the MW differs significantly from that of M31, not only in the number of 
clusters but also in their distribution over radius and metallicity. We compare these 
observations to the Auriga simulations in Section~\ref{sec:results_FeHRgc}.
...
\citet{2017MNRAS.465.3622R}. However, we caution that \DIFdelbegin \DIFdel{his }\DIFdelend \DIFaddbegin \DIFadd{this }\DIFaddend definition, although 
...
\begin{figure}
    ...
    \caption{
        ...
        The numbers in parenthesis show how many star particles are included 
        in the subset.
        \label{fig:Au4-10and21_FeH}
    }
\end{figure}
We select \mbox{Au4-10}\footnote{The nomenclature is `Au' for Auriga, followed by the
resolution level \DIFdelbegin \DIFdel{(}\DIFdelend 4 \DIFdelbegin \DIFdel{) }\DIFdelend and halo number \DIFdelbegin \DIFdel{(}\DIFdelend 10, indicating which set of initial
conditions was used\DIFdelbegin \DIFdel{)}\DIFdelend .}, \mbox{Au4-21}, and \mbox{Au4-4} to show the
distributions of three individual simulation runs that we consider representative
for specific behaviour, and to highlight that different runs give rise to 
different distributions. We show the age-selected GC candidates as well as the 
accreted and {\it in~situ} subsets. 
The top panel of Figure~\ref{fig:Au4-10and21_FeH} shows the MW 
...
$0.0$ to $-0.6$. This reflects the enrichment history of the galaxy because the
age cut removes the more metal-rich stellar population which shifts the mean
metallicity towards lower values. Furthermore, accreted GC candidates generally have
...
\mbox{Au4-1} is undergoing a major merger at redshift zero and we find a mean
metallicity in this simulation $\mu = -1.51 \, ({-0.74})$ for the {\it in~situ} 
...
[Fe/H]~\DIFdelbegin \DIFdel{$ = 1.5$}\DIFdelend \DIFaddbegin \DIFadd{$ = -1.5$}\DIFaddend . None of the Auriga subsets has a mean value that offers much 
...
\begin{figure}
    ...
    \caption{
        ...
        age $>$~10~Gyr; beige solid). The latter subset is further split up
...
\end{figure}
We now turn to our second goal, which is to see whether sufficient total mass in 
...
median over all the $30$~Auriga L4 haloes and the 25-75 percentile range as shaded regions, 
which shows the scatter between runs that have different initial 
conditions, thus, have varying merger histories. We chose to aggregate the data 
...
the combined efficiency would have to increase with decreasing 
...
metallicity. This trend could be consistent with the cluster formation efficiency 
model of \citet{2012MNRAS.426.3008K} as lower metallicity star particles formed 
...
\begin{figure}
    \includegraphics[width=\columnwidth]{{LogRgc_median_sigma-trim}.png}
    \caption{
        Median and standard deviation of the logarithm of radial distribution 
...
in all Auriga L4 galaxies. Figure~\ref{fig:Rgc_median_sigma} shows the median and 
standard deviation of $\log_{10}(r_{\text{gal}})$ of star particles in all Auriga
L4 simulations, respectively. The purple and magenta cross show the median
of the MW and M31 GCs, which are to be compared to the beige crosses. The solid
dot now shows our calculation of the median value of the `red' metal-rich population,
...
found in the Auriga galaxies. Furthermore, the median of the MW GCS lies roughly 
within the range of values produced by the Auriga simulations, whereas the median
of M31 is somewhat larger and slightly closer to typical median values of the 
...
component, for [Fe/H] as well as for $r_\text{gal}$ as can be seen in Figure~\ref{fig:FeH}
and Figure~\ref{fig:Rgc}. The total mass excess in 
...
which must have orbits with pericentres close to the galactic bulge or bar. In addition, 
...
\begin{figure}
    ...
    \caption{
        ...
        mass excess. The number in each bin shows the colour-coded value that is 
...
$M_{vir}-N_{\text{gal}}$ correlation found by \citet{2019arXiv190100900B}. However,
...
change when the age cut is lowered to 8 or 6 Gyr\DIFdelbegin \DIFdel{. 
GC candidates is not affected definition because our selection
function mostly includes star particles that have formed before roughly $z=2$}\DIFdelend . 
