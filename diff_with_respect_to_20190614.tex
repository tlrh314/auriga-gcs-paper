...
%DIF DEL AurigaGCS_20190614.tex   Mon Jun 24 11:07:39 2019
%DIF ADD AurigaGCS.tex            Fri Jul 12 16:09:28 2019
%
...
%DIF 48c48
%DIF < \graphicspath{{img/}}   % Set image path
%DIF -------
\graphicspath{{figures/}}   % Set figure path %DIF > 
%DIF -------
...
can produce a realistic globular cluster \DIFaddbegin \DIFadd{(GC) }\DIFaddend population at redshift zero. We
compare properties of the simulated star particles in the Auriga haloes with
catalogues of observations of the Milky Way \DIFdelbegin \DIFdel{and Andromeda }\DIFdelend \DIFaddbegin \DIFadd{(MW) and Andromeda (M31) }\DIFaddend globular
...
typical for the MW \DIFdelbegin \DIFdel{GCS}\DIFdelend \DIFaddbegin \DIFadd{globular cluster system (GCS)}\DIFaddend , although we observe a varying 
mass-excess for \DIFdelbegin \DIFdel{the }\DIFdelend different $R_{\text{GC}}$-[Fe/H] bins. This implies different 
...
the MW \DIFdelbegin \DIFdel{(}\DIFdelend \DIFaddbegin \DIFadd{or }\DIFaddend M31 \DIFdelbegin \DIFdel{) }\DIFdelend GCS. For any one of the Auriga haloes we reject the null hypothesis
...
MW GCS\DIFdelbegin \DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{, if cluster disruption is equally efficient regardless of metallicity.
...
the observations. The simulations yield realistic Milky Way analogues, and \DIFdelbegin \DIFdel{such
galaxies are known to }\DIFdelend \DIFaddbegin \DIFadd{real
galaxies }\DIFaddend host globular cluster systems. The question thus naturally arises whether 
the \DIFaddbegin \DIFadd{star formation histories of the }\DIFaddend simulations (also) \DIFdelbegin \DIFdel{yield a population of star particles that 
is consistent with }\DIFdelend \DIFaddbegin \DIFadd{give rise to a GC system 
similar to }\DIFaddend the Milky Way and/or Andromeda globular cluster systems. In particular, 
we \DIFdelbegin \DIFdel{investigate }\DIFdelend \DIFaddbegin \DIFadd{use }\DIFaddend the Auriga suite of cosmological zoom simulations 
(\citealt{2017MNRAS.467..179G}, and described in Sec.~\ref{sec:auriga}) \DIFdelbegin \DIFdel{because
a study of its GC system could serve as an ultimate test of }\DIFdelend \DIFaddbegin \DIFadd{to
investigate whether }\DIFaddend the star formation model \DIFdelbegin \DIFdel{. }\DIFdelend \DIFaddbegin \DIFadd{implemented produces metallicity, 
...
the mean for MW clusters \citep{2005ApJS..161..304M}. \DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <  \citet{2019AJ....157...12B} communicate the latest efforts to aggregate the
%DIF <  available data, presented in their CatClu catalog. Amongst 10978 star clusters
%DIF <  and alike objects in the Milky Way, the catalog contains 200 GCs and 94 GC
%DIF <  candidates. The CatClu catalog contains reference papers, positions, distances,
%DIF <  and total absolute V magnitude. Therefore we rely on the H96e10 dataset for all
%DIF <  other quantities, but
%DIFDELCMD < 
...
%DIF >  and alike objects in the Milky Way, the catalog contains 200 GCs and 94 GC
%DIF >  candidates. The CatClu catalog contains reference papers, positions, distances,
%DIF >  and total absolute V magnitude. Therefore we rely on the H96e10 dataset.
\DIFaddend 
...
\DIFaddbegin \subsection{Age-estimates}
\DIFadd{Figure~\ref{fig:MW-M31-age} shows a histogram of the age-estimates of the $55$
...
below $10$~Gyr. }\DIFaddend The M31 GCS has a mean age of $11.0 \pm 0.2$~Gyr with a dispersion 
of $2.2$~Gyr, and $24$ GCs have age-estimates below $10$~Gyr with a minimum age 
of $4.8$~Gyr. \DIFdelbegin \DIFdel{We show a histogram of the age-estimates of the $55$ MW 
GCs in V13 and $88$ GCs
in }\DIFdelend \DIFaddbegin \DIFadd{Based on these data, we find that the age distributions of the MW 
...
magnitude of the uncertainty is insufficient to explain the low-age tail in }\DIFaddend M31\DIFdelbegin \DIFdel{for which age-estimates are available in CR16, see Figure~\ref{fig:MW-M31-age}}\DIFdelend .
...
the MW and $17$ or 4.6\% in M31). \DIFaddbegin \DIFadd{Moreover, the M31 hosts more metal-rich 
(}[\DIFadd{Fe/H}]\DIFadd{$ > -1$) GCs in each radial bin in comparison to the MW GCSs than what
...
systems of the Milky Way and that of Andromeda both differ significantly. }\DIFaddend We 
...
metallicity distributions consistent with the MW \DIFdelbegin \DIFdel{(}\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend M31 \DIFdelbegin \DIFdel{) GC system}\DIFdelend \DIFaddbegin \DIFadd{GC systems}\DIFaddend , and whether
the subgrid generates sufficient total mass at metallicities typical for the
MW \DIFdelbegin \DIFdel{(}\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend M31 \DIFdelbegin \DIFdel{) GCS}\DIFdelend \DIFaddbegin \DIFadd{GCS's. To visually inspect the former we show the normalized metallicity 
distribution of three specific Auriga galaxies in Figure~\ref{fig:Au4-10and21_FeH}
in comparison to the MW and M31 }[\DIFadd{Fe/H}] \DIFadd{distributions}\DIFaddend .
...
\end{figure*}
\DIFdelbegin \DIFdel{Figure~\ref{fig:Au4-10and21_FeH} shows the normalized metallicity distribution
of }\DIFdelend \DIFaddbegin \DIFadd{We select }\DIFaddend \mbox{Au4-10}\footnote{The nomenclature is `Au' for Auriga, followed by the
resolution level (4) and halo number (10, indicating which set of initial
conditions was used the run).}, \mbox{Au4-21}, and \mbox{Au4-4} \DIFdelbegin \DIFdel{, showing the }\DIFdelend \DIFaddbegin \DIFadd{to show the
...
different distributions. We plot the age-selected }\DIFaddend GC candidates in green\DIFdelbegin \DIFdel{. The accreted GC candidates are shown }\DIFdelend \DIFaddbegin \DIFadd{, those
that are accreted }\DIFaddend in red, and the insitu subpopulation in blue. The top 
\DIFdelbegin \DIFdel{panel }\DIFdelend \DIFaddbegin \DIFadd{half }\DIFaddend of the left \DIFdelbegin \DIFdel{panel }\DIFdelend \DIFaddbegin \DIFadd{figure }\DIFaddend shows the MW (M31) GC system in purple (magenta)\DIFdelbegin \DIFdel{. We }\DIFdelend \DIFaddbegin \DIFadd{, where 
we }\DIFaddend overplot a double Gaussian for the MW GCS (the purple dashed lines) \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{by 
}\DIFaddend adopting literature values of the mean $\mu$ and standard deviation $\sigma$ of 
the metal-rich and metal-poor \DIFdelbegin \DIFdel{populations }\DIFdelend \DIFaddbegin \DIFadd{subpopulations }\DIFaddend \citep[][p. 38]{Harris2001}.

We find that the age cut, on average, lowers the mean metallicity from $0.0$ to
\DIFdelbegin \DIFdel{$-0.5$}\DIFdelend \DIFaddbegin \DIFadd{$-0.6$}\DIFaddend . Furthermore, the old accreted stars generally have lower \DIFaddbegin \DIFadd{mean }\DIFaddend metallicities
than the old insitu stars \DIFdelbegin \DIFdel{. The difference between the mean metallicity of the old insitu
and old accreted stars }\DIFdelend \DIFaddbegin \DIFadd{with differences of roughly ${\sim}0.3$ }\DIFaddend for the majority 
of the \DIFdelbegin \DIFdel{simulations is ${\sim}0.3$ (e.g. \mbox{Au4-21}), and }\DIFdelend \DIFaddbegin \DIFadd{simulation runs. This behaviour can be seen in \mbox{Au4-21}, while a 
slightly larger difference of }\DIFaddend ${\sim}0.5$ dex \DIFdelbegin \DIFdel{for \mbox{Au4-10}}\DIFdelend \DIFaddbegin \DIFadd{is seen for \mbox{Au4-10} (as plotted)}\DIFaddend , 
\mbox{Au4-16}, \mbox{Au4-17}, \mbox{Au4-18} and \mbox{Au4-22}. \DIFdelbegin \DIFdel{This trend 
is only }\DIFdelend \DIFaddbegin \DIFadd{However, this trend 
is }\DIFaddend reversed for \mbox{Au4-1} and \mbox{Au4-4} \DIFdelbegin \DIFdel{, }\DIFdelend for which the old insitu population 
has a lower mean metallicity instead. \DIFdelbegin \DIFdel{For \mbox{Au4-1} }\DIFdelend \DIFaddbegin \DIFadd{We caution that \mbox{Au4-1} is undergoing
a major merger at redshift zero and note that }\DIFaddend we find $\mu = -1.51 \, ({-0.74})$ 
for the old insitu (accreted) \DIFdelbegin \DIFdel{,
but }\DIFdelend \DIFaddbegin \DIFadd{although }\DIFaddend the former consists of only $1019$ 
particles ($1.3$\% of all GC candidates, and with a total mass of 5e7 M\Sun)\DIFdelbegin \DIFdel{thus we believe our classification of insitu is
flawed for this halo due to misclassification of the primary halo in the merger
tree by subfind}\DIFdelend . 
...
in Figure~\ref{fig:FeH_mu_sigma} \DIFaddbegin \DIFadd{to show the behaviour for the full set of Auriga
galaxies}\DIFaddend . The green crosses are to be compared to the purple (magenta) cross, 
which shows the mean value of all MW (M31) GCs. \DIFdelbegin \DIFdel{We }\DIFdelend \DIFaddbegin \DIFadd{In addition, we show the metal-rich 
...
observational data cut at }[\DIFadd{Fe/H}]\DIFadd{~$= -1$. We do not include corresponding data 
...
concerning uni- bi- or trimodality in the }[\DIFadd{Fe/H}] \DIFadd{distribution, but CR16 argues 
that the data, after removal of younger objects due to improved age classification, 
hints at three populations separated at }[\DIFadd{Fe/H}]\DIFadd{~$>-0.4$ and }[\DIFadd{Fe/H}]\DIFadd{~$ < 1.5$. None 
...
is reached concerning uni- or bimodality in the }%DIFDELCMD < [%%%
...
\end{figure}
We now turn to the \DIFaddbegin \DIFadd{second goal, which is to see whether sufficient }\DIFaddend total mass in 
GC candidates produced by the Auriga simulations. \DIFaddbegin \DIFadd{To answer this question we plot
a mass-weighted metallicity distribution in Figure~\ref{fig:FeH}. }\DIFaddend We show the 
...
to aggregate the data to indicate \DIFdelbegin \DIFdel{a general trend }\DIFdelend \DIFaddbegin \DIFadd{general trends }\DIFaddend that we find when the GC candidates 
are split up according to birth location, rather than selecting typical examples 
of individual (simulated) galaxies. \DIFdelbegin \DIFdel{Figure~\ref{fig:FeH} shows 
the mass-weighted metallicity distribution of all Auriga L4 galaxies. }\DIFdelend Once again we notice that the peak metallicity 
shifts down from $0$ to \DIFdelbegin \DIFdel{$-0.5$ }\DIFdelend \DIFaddbegin \DIFadd{$-0.6$ }\DIFaddend for old stars (green solid) compared to all stars 
...
value. We show the MW (M31) GCS in purple (magenta) and \DIFdelbegin \DIFdel{we }\DIFdelend \DIFaddbegin \DIFadd{notice that the difference 
between the MW and M31 distributions is substantially larger than the scatter
between different Auriga galaxies, particularly around }[\DIFadd{Fe/H}]\DIFadd{~$=-1$. In addition, 
Andromeda does host (a hand full) GCs with }[\DIFadd{Fe/H}]\DIFadd{~$< -2.5$ as well as GCs with 
}[\DIFadd{Fe/H}]\DIFadd{~$ > 0$ while the Milky Way does not.
}

\DIFadd{We }\DIFaddend show the ratio of the  simulated to the observed profiles in the middle (bottom) 
...
while still producing \DIFdelbegin \DIFdel{a GC system with sufficient mass to be consistent
with the MW (M31) GCS. The }\DIFdelend \DIFaddbegin \DIFadd{sufficient mass at the right metallicities. In particular,
the }\DIFaddend cluster formation efficiency would have to linearly decrease with decreasing 
...
\end{figure}
\DIFaddbegin \DIFadd{We repeat the analysis of the previous subsection for the distribution of
galactocentric radius instead of metallicity. We look for general trends present
in all Auriga L4 galaxies. }\DIFaddend Figure~\ref{fig:Rgc_mu_sigma} shows the mean and 
standard deviation of the radial distribution of star particles in all Auriga L4 
simulations. We notice that the old insitu populations are \DIFdelbegin \DIFdel{much }\DIFdelend more centrally 
distributed, whereas the old accreted \DIFdelbegin \DIFdel{component has }\DIFdelend \DIFaddbegin \DIFadd{subsets have }\DIFaddend a larger radial extent. \DIFdelbegin \DIFdel{Moreover, }\DIFdelend \DIFaddbegin \DIFadd{This 
is not surprising because the classification of in situ requires star particles 
...
we notice that }\DIFaddend the dispersion increases with increasing mean value of the radial 
distribution as may be expected for a non-negative quantity. \DIFaddbegin \DIFadd{The purple (magenta)
cross indicates the values for the MW (M31) GCS, which are to be compared to the
green crosses. The MW GCS lies roughly within the range of values seen for the 
radial distributions produced by the Auriga simulations, whereas the mean of M31
is larger and closer to typical mean values of the accreted GC candidates in
the Auriga simulations. The larger radial extend of the M31 GCS is generally
believed to hint at a more accretion-dominated origin of the GCS, and may reflect
a richer accretion history of the galaxy in comparison to the Milky Way.
}\DIFaddend 
...
to the accreted subpopulation. \DIFaddbegin \DIFadd{For the Milky Way we notice that significantly
fewer GCs are found beyond $40$~kpc than for M31, and that accreted GC candidates
contribute mostly to the Auriga GCS at these radii. }\DIFaddend We further investigate a 
...
\DIFaddbegin \DIFadd{We notice in Figure~\ref{fig:Au-FeHRgc} that the Auriga simulations, on average,
...
Figure~\ref{fig:Au-FeHRgc-ratio}) shows a less clear trent than for the Milky Way. 
...
%DIF <  \todo[inline]{
%DIF <      Harris: `Somewhat arbitrarily, I will take the region $r_p > 3$ kpc
%DIF <      (containing 75 clusters) as the fiducial Milky Way sample. If we were to
%DIF <      view the Milky Way at the same inclination angle to the disk as we see
%DIF <      M31, this cutoff in projected distance would correspond roughly to the
%DIF <      inner distance limits in the M31 halo sample.'
%DIF <  }
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < \subsection{Star particles are not globular clusters}
%DIFDELCMD < \label{sec:discussion_mass_excess}
%DIFDELCMD < %%%
%DIF <  A major improvement to our classification of star particles as GC candidates
%DIF <  could be made by explicitly taking into the formation efficiency of bound star
%DIF <  clusters \citep{2012MNRAS.426.3008K}, which depends on the local ISM properties
%DIF <  (density, pressure, sound speed). However, the only information available in
%DIF <  post-processing are the properties of the star particles recorded in the first
%DIF <  snapshot after formation. We could attempt to obtain the properties of the nearest
%DIF <  gas cell, but this would have to be limited to a very narrow time window between
%DIF <  the time of formation and the time of the snapshot because the star particle could
%DIF <  otherwise be in a completely different location than where it was born initially.
%DIF <  Moreover, even if the birth time and snapshot time would coincide, it would not
%DIF <  be obvious that the gas cell will yield the correct quantities as the formation
%DIF <  of the star itself alters the properties of the gas. We therefore did not further
%DIF <  attempt a more sophisticated modeling of the cluster formation efficiency in
%DIF <  post-processing.
%DIF <  
%DIF <  Furthermore, we compare star particles to present-day globular clusters, thereby
%DIF <  ignoring the effects of (dynamical) disruption of globular clusters over nearly
%DIF <  a Hubble time. As shown by \citep{2018MNRAS.475.4309P}, a detailed model of the tidal
%DIF <  history of star clusters requires a temporal resolution of order mega year. For
%DIF <  the Auriga L4 simulations we have a total of 128 snapshots available over the
%DIF <  age of the Universe, thus, far too coarse temporal resolution for meaningful
%DIF <  calculations of tidal disruption along the orbit of our GC candidates. Therefore
%DIF <  we can only combine these two shortcomings of our analysis into the `mass excess',
%DIF <  which includes the cluster formation efficiency and consecutive globular cluster
%DIF <  disruption.
%DIFDELCMD < 

%DIFDELCMD < \todo[inline]{Paraphrase the following}
%DIFDELCMD < %%%
...
of massive (M $>10^5$ M}%DIFDELCMD < \Sun%%%
\DIFdel{) clusters with }%DIFDELCMD < [%%%
\DIFdel{Fe/H}%DIFDELCMD < ]%%%
...
}\DIFdelend %DIF >  Copy-pasted from \citet{2019MNRAS.486.5838R}
