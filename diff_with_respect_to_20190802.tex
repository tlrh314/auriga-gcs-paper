...
%DIF ADD AurigaGCS.tex            Tue Aug 20 11:33:02 2019
%
...
\begin{abstract}
We investigate whether the galaxy \DIFaddbegin \DIFadd{and star }\DIFaddend formation model used for the Auriga simulations
can produce a realistic globular cluster (GC) population\DIFdelbegin \DIFdel{at redshift zero}\DIFdelend . We compare \DIFdelbegin \DIFdel{properties of the simulated }\DIFdelend \DIFaddbegin \DIFadd{statistics 
of GC candidate }\DIFaddend star particles in the Auriga haloes with catalogues of \DIFdelbegin \DIFdel{observations of }\DIFdelend the Milky 
Way (MW) and Andromeda (M31) globular cluster populations\DIFdelbegin \DIFdel{available in the literature}\DIFdelend . We find that the Auriga 
simulations do produce sufficient \DIFdelbegin \DIFdel{mass }\DIFdelend \DIFaddbegin \DIFadd{stellar mass for GC candidates }\DIFaddend at radii and 
metallicities that are typical for the MW \DIFdelbegin \DIFdel{globular cluster }\DIFdelend \DIFaddbegin \DIFadd{GC }\DIFaddend system (GCS), although we observe a 
varying \DIFdelbegin \DIFdel{mass-excess for different $R_{\text{GC}}$}\DIFdelend \DIFaddbegin \DIFadd{mass-ratio for different galactocentric radius-metallicity 
($R_{\text{gal}}$}\DIFaddend -[Fe/H]\DIFdelbegin \DIFdel{bins}\DIFdelend \DIFaddbegin \DIFadd{) bins of simulated GC candidates relative to the 
observed mass in the MW and M31 GC systems}\DIFaddend .
This implies different values for the combined product of the bound cluster 
formation efficiency and the \DIFdelbegin \DIFdel{globular cluster }\DIFdelend \DIFaddbegin \DIFadd{GC }\DIFaddend disruption rate. Furthermore we test whether any of the 
...
the \DIFdelbegin \DIFdel{Milky Way as well as that of the Andromeda galaxy}\DIFdelend \DIFaddbegin \DIFadd{MW and M31}\DIFaddend . The same holds true for the distribution of \DIFdelbegin \DIFdel{galactocentric radius}\DIFdelend \DIFaddbegin \DIFadd{$R_{\text{gal}}$}\DIFaddend .
Overall, the Auriga simulations produce \DIFdelbegin \DIFdel{old star particles }\DIFdelend \DIFaddbegin \DIFadd{GC candidates }\DIFaddend with higher metallicities 
than the MW and M31 GCS and at larger radii. \DIFdelbegin \DIFdel{The }\DIFdelend \DIFaddbegin \DIFadd{This implies that the bound cluster
}\DIFaddend formation efficiency would have to \DIFdelbegin \DIFdel{linearly }\DIFdelend decrease with increasing metallicity for the 
Auriga GC candidates to be consistent with the MW GCS, if \DIFdelbegin \DIFdel{cluster }\DIFdelend \DIFaddbegin \DIFadd{GC }\DIFaddend disruption is equally 
...
single stellar population with a total mass that could be consistent with one 
globular cluster. This means that their formation sites and that of a real-world 
GC may be consistent.
...
We find that the age cut, on average, lowers the mean metallicity \DIFaddbegin [\DIFadd{Fe/H}] \DIFaddend from 
...
Moreover, none of the simulations has a \DIFaddbegin \DIFadd{(total) }\DIFaddend population of GC 
candidates with a bimodal metallicity distribution\DIFdelbegin \DIFdel{(the green
curves)}\DIFdelend .
...
can be compared to \DIFdelbegin \DIFdel{Figure~\ref{fig:observations_FeHRgc}). Finally, we divide these value by the total
...
of Figure~\ref{fig:Au-FeHRgc-ratio} for mass excess with respect to the MW (M31)GCS. }%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{We notice in Figure~\ref{fig:Au-FeHRgc} }\DIFdelend \DIFaddbegin \DIFadd{the observed total GC mass shown in 
Figure~\ref{fig:observations_FeHRgc}). We notice }\DIFaddend that the Auriga simulations, 
on average, produce fewer GCs in the lowest metallicity bins ($-2.5$ to $-1.5$) 
than at higher metallicities in the three innermost radial bins. In addition, 
Auriga produces fewer high-metallicity GCs at large radii, similar to the MW 
and M31 \DIFdelbegin \DIFdel{GC systems. However, the three bins in the upper right corner are populated in }\DIFdelend \DIFaddbegin \DIFadd{GCSs. 
}

\DIFadd{To ease the comparison between the simulations and observations we take the ratio
...
Note that the data in three top right bins is masked because the mass ratio cannot 
...
indicate a higher and lower than average mass excess. In particular, the median 
value used to shift the mass ratio is $2.68$ and $1.69$ dex for the MW and M31, 
respectively. This means that the overall `mass budget' that the simulations `can 
afford to lose' is roughly a factor $480$ and $50$ for the MW and M31, respectively. 
The lowest mass surplus for both galaxies is the the bin where $-2.5 <$ }[\DIFadd{Fe/H}] 
...
more mass in GC candidates than what is observed in }\DIFaddend the \DIFdelbegin \DIFdel{Auriga
simulations , in contrast to the observations.
The }\DIFdelend \DIFaddbegin \DIFadd{MW and M31 respectively.
...
be consistent with the \DIFdelbegin \DIFdel{Milky Way }\DIFdelend \DIFaddbegin \DIFadd{MW or M31 }\DIFaddend GCS.
