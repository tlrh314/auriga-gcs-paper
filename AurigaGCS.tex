...
the subgrid generates sufficient total mass at metallicities (and radii) typical
for the MW (M31) GCS.

\begin{figure*}
    \includegraphics[width=\columnwidth]{{Au4-10_FeH_cleaner-trim}.png}
    \includegraphics[width=\columnwidth]{{Au4-21_FeH_cleaner-trim}.png}
    \caption{
        \emph{Left:} Metallicity distribution of Au4-10 (bottom panel). We show 
        the the GG candicates in green. We split the GC candidates into two 
        subpopulations, those that have formed insitu (blue), and those that have 
        been accreted (red). The thin green line shows all star particles.
...
of Au4-10\footnote{The nomenclature is `Au' for Auriga, followed by the resolution 
level (4) and halo number (10 - indicating which set of initial conditions was 
used the run).} and Au4-21, showing the GC candidates in green. The accreted GC 
...
We find that the age cut lowers the mean metallicity from $\sim 0.0$ to $\sim -0.6$.
Furthermore, the old accreted stars generally have lower metallicities than the
old insitu stars. The difference between the mean metalicity of the old insitu 
and old accreted stars for the majority of the simulations is $\sim 0.3$ (e.g.
Au4-21), and $\sim 0.5$ dex for Au4-10, Au4-16, Au4-17, Au4-18 and Au4-22.
This trent is only reversed for Au4-1 and Au4-4, for which the old insitu population
has a lower mean metallicity instead. For Au4-1 we find $\mu = -1.51$ (-0.74) 
for the old insitu (accreted), but the former consists of only 1019 particles 
(with a total mass of 5e7 M\Sun). Overall we find that the simulations produce 
(sub)populations of GC candidates that are more metal-rich than the MW and M31 
GC systems. Moreover, none of the simulations has a population of GC candidates 
with a bimodal metallicity distribution (the green curves).
...
\begin{figure}
    ...
    \caption{
        ...
        Each cross represents one simulation for a given colour. The green (blue) 
...
except when comparing the old accreted stars of Au4-10 to the MW GCS. In this 
case we still reject $H_0$ with, but with $\leq 98.68$~\% confidence. When comparing
to the M31 GCS, the `best' matches are the old accreted subpopulations of Au4-13,
Au4-15, and Au4-17 ($H_0$ rejected with $\leq$ 99.82~\% confidence, 99.95~\%, 
and 98.32~\%). Here we find that the cumulative distributions are more or less 
similar up to [Fe/H]~$\sim$~-0.5, above which the simulations yield higher number 
...
than the MW (M31) GCS because under-production of old stars with [Fe/H]~$\leq$~-1.5,
and over-production of stars with [Fe/H]$\geq$~-0.5, are the main reason reasons
that $H_0$ is rejected with such high confidence.
...
simulations show larger dispersions.
...
the peak metallicity shifts down from $0$ to $-0.5$ for old stars (green solid)
compared to all stars (green dotted) while the mass at the peak lowers by 
