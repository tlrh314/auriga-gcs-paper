...
\begin{figure*}
    \includegraphics[width=\columnwidth]{{Au4-24_FeH_cleaner-trim}.png}
    \includegraphics[width=\columnwidth]{{Au4-24_FeH_mass-trim}.png}
    \caption{
        \emph{Left:} Metallicity distribution of Au4-24 (bottom panel). We show 
        the the GG candicates in orange. We split the GC candidates into two 
        subpopulations, those that have formed insitu (blue), and those that have 
        been accreted (red). The thin orange line shows all star particles.
        The solid purple (magenta) line in the top panel shows the GC system 
        of the MW (M31). \emph{Right:} Total mass in each metallicity bin.
        \label{fig:Au4-24_FeH}
    }
\end{figure*}
Figure~\ref{fig:Au4-24_FeH} shows the metallicity distribution of 
Au4-24\footnote{The nomenclature is `Au' for Auriga, followed by the resolution 
level (4) and halo number (24 - indicating which set of initial conditions was used 
the run).} as one particular example of an Auriga galaxy. The panel on the left
shows the distribution normalized by the number of particles, showing the GC
candidates in orange. The accreted GC candidates are shown in red, and the insitu
subpopulation is shown in blue. The top panel shows the MW (M31) GC system in
purple (magenta). We overplot two Gaussian curves (the purple dashed lines) using
literature values of the mean $\mu$ and standard deviation $\sigma$ of the 
metal-rich and metal-poor populations of the MW GCS \citep[][p. 38]{Harris2001}.
However, we use arbitrary normalizations to produce aesthetically pleasing curves.
The panel on the right shows the metallicity distribution weighted by the total
mass of the particles (observations) in that bin.


We find that the metallicity distribution of the Auriga simulations blabla.
