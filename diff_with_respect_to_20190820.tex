...
%DIF ADD AurigaGCS.tex            Wed Sep  4 19:06:32 2019
%
...
\DIFadd{$R_{\text{vir}}$ is the radius in a spherical `top-hat' perturbation model where
the average density equals $\Delta_{\text{vir}}=97.4$ times the critical 
density of the Universe (WMAP9), using the values 
and definition from \mbox{%DIFAUXCMD
\citet{2017MNRAS.464.3825P}}\hspace{0pt}%DIFAUXCMD
.}} \DIFadd{$R_{\text{vir,M31}} = 299$~kpc
and multiply by $R_{\text{vir,MW}} = 261$~kpc to compensate for the different
intrinsic sizes of both galaxies. Values for R$_{\text{gal}}$ }\DIFaddend are readily available 
in H96e10 (assuming $R_{\odot}=8.0$~kpc), but the galactocentric radius of GCs
