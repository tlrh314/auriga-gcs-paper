...
%DIF DEL AurigaGCS_20190820.tex   Tue Aug 20 16:28:59 2019
%DIF ADD AurigaGCS.tex            Thu Sep  5 20:29:55 2019
%
...
(\citealt{2014MNRAS.437.1750M}; \citetalias{2017MNRAS.467..179G}). Galactic 
...
The galaxy formation model produces realistic spiral galaxies at redshift \DIFdelbegin \DIFdel{$z=0$.
Moreover,
Auriga reproduces the }\DIFdelend \DIFaddbegin \DIFadd{zero
that match observations such as sizes, morphologies (\mbox{%DIFAUXCMD
\citetalias{2017MNRAS.467..179G}}\hspace{0pt}%DIFAUXCMD
),
}\DIFaddend HI disc properties \citep{2017MNRAS.466.3859M}, \DIFdelbegin \DIFdel{the chemical thin/thick disc \mbox{%DIFAUXCMD
\citep{2018MNRAS.474.3629G}}\hspace{0pt}%DIFAUXCMD
, the diversity in 
}\DIFdelend \DIFaddbegin \DIFadd{and the diversity of }\DIFaddend accreted stellar
halo properties \DIFdelbegin \DIFdel{found in observations \mbox{%DIFAUXCMD
\citep{2019MNRAS.485.2589M}}\hspace{0pt}%DIFAUXCMD
,
and the }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{2019MNRAS.485.2589M}}\hspace{0pt}%DIFAUXCMD
. Furthermore, the simulations are able to
reproduce major Galactic components, such as the chemical thin/thick disc dichotomy
\mbox{%DIFAUXCMD
\citep{2018MNRAS.474.3629G} }\hspace{0pt}%DIFAUXCMD
and the recently discovered }\DIFaddend \textit{Gaia} Sausage 
\citep{2019MNRAS.484.4471F}. \DIFdelbegin \DIFdel{These last two 
bolster the point that the simulations are similar (and detailed ) }\DIFdelend \DIFaddbegin \DIFadd{They are thus a large set of detailed simulations
similar }\DIFaddend enough to the \DIFdelbegin \DIFdel{MW to reproduce major, distinct features of the Galaxy , and so looking }\DIFdelend \DIFaddbegin \DIFadd{Galaxy to make predictions }\DIFaddend for the GC population properties
\DIFdelbegin \DIFdel{and comparing them to the MW /}\DIFdelend \DIFaddbegin \DIFadd{of the MW and }\DIFaddend M31\DIFdelbegin \DIFdel{is all the more relevant}\DIFdelend .
...
of the MW and M31 GCS. \DIFdelbegin \DIFdel{Both quantities }\DIFdelend \DIFaddbegin \DIFadd{We divide the radii of M31 by its virial radius}\footnote{
\DIFadd{$r_{\text{vir}}$ is the radius in a spherical `top-hat' perturbation model at which
the average density reaches an overdensity of $\Delta_{\text{vir}}=357$ times the 
background density, adopting the values and cosmology from \mbox{%DIFAUXCMD
\citet{2017MNRAS.464.3825P}}\hspace{0pt}%DIFAUXCMD
}}
\DIFadd{$r_{\text{vir,M31}} = 299$~kpc and multiply by $r_{\text{vir,MW}} = 261$~kpc to 
account for the different intrinsic sizes of both galaxies. Values for 
R$_{\text{gal}}$ }\DIFaddend are readily available in H96e10 (assuming \DIFdelbegin \DIFdel{$R_{\odot}=8.0$}\DIFdelend \DIFaddbegin \DIFadd{$r_{\odot}=8.0$}\DIFaddend ~kpc), but the galactocentric radius of GCs
in M31 is not available in CR16. Therefore, we follow \citet[][Sec.~4.1]{2019arXiv190111229W} 
to calculate the projected radius \DIFdelbegin \DIFdel{$R_{\text{proj}}$ }\DIFdelend \DIFaddbegin \DIFadd{$r_{\text{proj}}$ }\DIFaddend from the observed positions,
...
We calculate \DIFdelbegin \DIFdel{$R_{\text{gal}}$ }\DIFdelend \DIFaddbegin \DIFadd{$r_{\text{gal}}$ }\DIFaddend as `average deprojected distance`
\DIFdelbegin \DIFdel{$R_{\text{gal}} = R_{\text{proj}} \times (4/\pi)$}\DIFdelend \DIFaddbegin \DIFadd{$r_{\text{gal}} = r_{\text{proj}} \times (4/\pi)$}\DIFaddend . 
...
distributions show a similar trend for \DIFdelbegin \DIFdel{$R_{\text{gal}} > 4$}\DIFdelend \DIFaddbegin \DIFadd{$r_{\text{gal}} > 4$}\DIFaddend ~kpc and host a 
...
\begin{figure}
    ...
    \caption{
        ...
        (i.e. the same sample as in the top panel). \DIFaddbeginFL \DIFaddFL{We divide the radii of M31 by
        $r_{\text{vir,M31}} = 299$~kpc and multiply by $r_{\text{vir,MW}} = 261$~kpc 
        \mbox{%DIFAUXCMD
\citep{2017MNRAS.464.3825P} }\hspace{0pt}%DIFAUXCMD
to compensate for the different intrinsic 
        sizes of both galaxies. 
        }\DIFaddendFL \label{fig:MW-M31-age-Rgc}
    }
\end{figure}
...
radii in general (\DIFdelbegin \DIFdel{$R_{\text{gal}} > 30$}\DIFdelend \DIFaddbegin \DIFadd{$r_{\text{gal}} > 30$}\DIFaddend ~kpc; right column: $11$ GCs or 7.3\% in
...
\begin{figure}
    ...
    \caption{
        \emph{Top}: Mass-weighted \DIFdelbeginFL \DIFdelFL{$R_{\text{gal}}$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$r_{\text{gal}}$}\DIFaddendFL -[Fe/H] distribution of
        ...
        \citep[data from][]{2016ApJ...824...42C}. \DIFaddbeginFL \DIFaddFL{Radii of M31 are again divided by 
        $r_{\text{vir,M31}}$ and multiplied by $r_{\text{vir,MW}}$. }\DIFaddendFL Note that the 
...
\begin{figure}
    ...
    \caption{
        ...
        candicates in green, and use a green dotted line for all star particles. 
...
accreted and {\it in~situ} subsets. The top half of the left figure shows the MW 
...
\begin{figure}
    ...
    \caption{
        ...
        [red] crosses show the values calculated using all (only {\it in~situ})
        [only accreted] GC candidates. Green triangles indicate that all stars
...
{\it in~situ} GC candidates appear roughly consistent with that of the metal-rich 
...
\begin{figure}
    ...
    \caption{
        ...
        stars (green dotted) and GC candidates (i.e. stars with
...
median for all thirty Auriga L4 haloes with the $1\sigma$ interval around it 
...
[Fe/H]~=~$-1$, and by the accreted subset below this value. We show the MW (M31) 
...
galaxies, particularly around [Fe/H]~$=-1$. In addition, M31 does host 
...
\begin{figure}
    ...
    \caption{
        ...
        the MW (M31) GCS shown in purple (magenta). \DIFaddbeginFL \DIFaddFL{The Auriga (M31) radii are 
        divided by $r_{\text{vir,Au}}$ ($r_{\text{vir,M31}}$) and multiplied 
        by $r_{\text{vir,MW}}$ to compensate for different intrinsic sizes of
        the galaxies.
        }\DIFaddendFL \label{fig:Rgc_mu_sigma}
    }
\end{figure}
We repeat the analysis of the previous subsection for the distribution of
galactocentric radius instead of metallicity. We \DIFaddbegin \DIFadd{divide the Auriga radii by
$r_{\text{vir,Au}}$ and multiply by $r_{\text{vir,MW}}$ to compensate for the different
intrinsic sizes of the Auriga galaxies. We }\DIFaddend look for general trends present
in all Auriga L4 galaxies. Figure~\ref{fig:Rgc_mu_sigma} shows the mean and 
standard deviation of \DIFdelbegin \DIFdel{$\log_{10}(R_{\text{gal}})$ }\DIFdelend \DIFaddbegin \DIFadd{$\log_{10}(r_{\text{gal}})$ }\DIFaddend of star particles in all Auriga
...
shows our calculation of the mean value of the `red' metal-rich (`blue` metal-poor) 
population. We split the radii up into the metal-rich/metal-poor groups by taking
...
we find that the simulations have a wider dispersion in \DIFdelbegin \DIFdel{$\log(R_{\text{gal}})$ }\DIFdelend \DIFaddbegin \DIFadd{$\log(r_{\text{gal}})$ }\DIFaddend than
...
\begin{figure}
    ...
    \caption{
        ...
        L4 simulations\DIFaddbeginFL \DIFaddFL{, where the Auriga (M31) radii are again divided by 
        $r_{\text{vir,Au}}$ ($r_{\text{vir,M31}}$) and multiplied by 
        $r_{\text{vir,MW}}$}\DIFaddendFL . We show the median value of all Auriga haloes for all
        stars (green dotted) and GC candidates (i.e. stars with
        age $>$~10~Gyr; green solid). The latter sub set is further split up
...
\begin{figure}
    ...
    \caption{
        Mass-weighted [Fe/H]-\DIFdelbeginFL \DIFdelFL{$R_{\text{gal}}$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$r_{\text{gal}}$ }\DIFaddendFL distribution of all thirty Auriga
